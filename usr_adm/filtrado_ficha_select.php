...
$segmentoInterno= "id, descriptor, fecha ";
...
*/
$sql ="SELECT $segmentoInterno $segmentoExterno FROM $estaVista $clausulaFecha $clausulaWhere ORDER BY id DESC ";
/*
	...
	if ($result->num_rows> 0){
		while ($row = $result->fetch_assoc()){
			...
			foreach ($row as $key => $value) {
				...
				$modalVer ='
				<form>
				<button type="button" class="btn btn-info btn-sm" id="verFicha_'.$i.'" onclick="verFicha('.$row['id'].')">Ver</button>
				</form>';
				$modalVer.='
