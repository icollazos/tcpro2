...
$nombrePagina='Gráficas de sectores para '.$esteAmbito;
...
//$estaVista= 'v_aaa_gasTotales';

$sql="SELECT column_name
FROM INFORMATION_SCHEMA.COLUMNS
WHERE table_name = '$estaVista';";
#p($P,$sql);
...
function desplegable($id,$titulo,$array){
	asort($array);
	$Z='<div class="col col-sm-3"><label class="form-label">'.$titulo.'</label><select multiple class="form-select alto argumento-select campos" id="'.$id.'" onblur="creaMarco();"><option value="">Seleccione...</option>';
...
<script type="text/javascript">
	...
	function tablita(titulo,array,aliasTitulo){
		var tablita='<div class="col col-lg-6 col-md-6 col-sm-6 col-xs-12"><div class="card card-info" style="margin-bottom:14px;"><div class="card-header">'+aliasTitulo+'</div><div class="card-body"><figure class="highcharts-figure"><div id="grafico_'+titulo+'"></div></figure>';
		/*
	...
	function creaMarco(){
		...
		$.post("filtrado_consulta_torta_select.php", {
		...
		.done(function(response){
			...
			$.each(j, function(campo){
				var estaTabla=j[campo];
				titulo=estaTabla['Titulo'];
				data=estaTabla['data'];
			...
			$.each(j, function(campo){
				...
				titulo=estaTabla['Titulo'];
				data=estaTabla['data'];
				totalOtros=estaTabla['Otros'];
				totalGeneral=estaTabla['Total'];
				contenedor='grafico_'+titulo;
				titulo=titulo;
				dataT=[];
				q=0;
				$.each(data, function(item){
					dataT[q]={name: item, y: data[item]};
					q++;
				});
				torta(contenedor,titulo,dataT,totalOtros,totalGeneral);
			});			
		})
		.fail(function() {
...
<script type="text/javascript">
	function torta(contenedor,titulo,data,totalOtros,totalGeneral){
