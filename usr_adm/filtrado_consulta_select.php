...
if(isset($arg['tipoBusqueda'])){
	...
	if(isset($arg['anoInicio']) AND isset($arg['anoFinal']) AND isset($arg['mesInicio']) AND isset($arg['mesFinal']) ){
		...
		$clausulaFecha=" WHERE v_aaa_texto.fecha >= '$fechaInicio' AND v_aaa_texto.fecha <= '$fechaFinal' ";
	}


	$arrayValores=$arg['valor'];
	if($arrayValores[0]!=0){
...
$sql ="SELECT $segmentoInterno $segmentoExterno FROM $estaVista $clausulaFecha $clausulaWhere $clausulaValor $clausulaEx ORDER BY id DESC LIMIT 0,5";
/*
j($sql);
			...
			foreach ($row as $key => $value) {
				...
				foreach ($row as $key => $value) {
					...
					if(substr($key, 0,3)!='id_'){
						if($aliasGeneral[$key]){ $t=$aliasGeneral[$key]; } else { $t= $key; }
						$tablaModal.='<tr><td>'.$t.'</td><td>'.$value.'</td></tr>';
					}
				}
				...
				<input type="hidden" name="from" value="filtrado_consulta.php?esteAmbito='.$esteAmbito.'">
