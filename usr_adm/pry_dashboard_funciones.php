...
function cargaSelects($argumentos){
	...
	foreach ($camposExternos as $k=>$v) {
		...
		$t[$i]['id']=$k;
		$t[$i]['alias']=$v;
		$sql="SELECT DISTINCT($k) as id, $campoDescriptor as value FROM $vista";
		$sqla[]=$sql;
...
function buscaDatos($argumentos){
	...
	switch ($fuente) {
		case 'totalMedios':
			$titulos=$argumentos['titulos'];
			$registros=buscaRegistros($esteAmbito,$argumentos,$fuente,$titulos);
			break;
		default:
			$titulos=$argumentos['titulos'];
			$registros=buscaRegistros($esteAmbito,$argumentos,'lemaPar',$titulos);
			break;
	}
...
function buscaRegistros($esteAmbito, $argumentos, $fuente, $titulos){
	$mysqli=conectar($_SESSION['datosConexion']);
	...
	$clausulaCampos=$argumentos['clausulaCampos'];
	$clausulaGroup=$argumentos['clausulaGroup'];
	$cliente=$_SESSION['cliente'];

	switch ($fuente) {
		case 'totalMedios':
			$sql="SELECT $clausulaCampos FROM $vista $clausulaFecha $clausulaKeywords $clausulaExcepcion $clausulaFiltros $clausulaCliente AND $vista.id>1 $clausulaGroup LIMIT 0,500";
			break;	
		case 'lemaPar':
			...
			$clausulaFecha=" $vista.fecha >= '$fechaInicio' AND $vista.fecha <= '$fechaFinal' ";

			$sql="SELECT  $clausulaCampos FROM aaa_lemaPar INNER JOIN aaa_texto ON aaa_texto.id=aaa_lemaPar.idaaa_texto $clausulaFecha $clausulaCliente AND aaa_lemaPar.id>1 GROUP BY  LIMIT 0,500";
			$sql="SELECT aaa_lemaPar.descriptor as Palabra, SUM(relevancia) as Relevancia  
			FROM aaa_lemaPar 
			INNER JOIN v_aaa_texto ON v_aaa_texto.id=aaa_lemaPar.idaaa_texto 
			WHERE (1=1) AND aaa_lemaPar.id>1 AND aaa_lemaPar.lop='L' 
			/*
			*/ 
			AND $clausulaFecha 
			AND v_aaa_texto.id_cliente_proyecto='$cliente'
			GROUP BY Palabra ORDER BY Relevancia DESC LIMIT 0,10";
			break;	
		default:
			# code...
			break;
...
function buscaTitulos($esteAmbito,$fuente){
	$mysqli=conectar($_SESSION['datosConexion']);
	$sql = "SHOW COLUMNS FROM v_aaa_$esteAmbito";
	$resultado = $mysqli->query($sql);
