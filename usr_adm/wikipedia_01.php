...
class Grafo {
    ...
    public function obtenerNodosEnCaminosMasCortos($inicio, $fin, $n) {
        $caminos = [];
        $this->buscarCaminos($inicio, $fin, [], $caminos, 0, $n);
        
        // Extrae solo los nodos de los caminos encontrados
        $nodosEnCaminos = [];
        foreach ($caminos as $camino) {
            $nodosEnCaminos[] = $camino['nodos'];
        }
        
        return array_slice($nodosEnCaminos, 0, $n); // Retorna solo los N caminos
    }
    ...
    private function buscarCaminos($actual, $fin, $caminoActual, &$caminos, $pesoActual, $n) {
        ...
        if ($actual === $fin) {
            $caminos[] = ['nodos' => $caminoActual, 'peso' => $pesoActual];
            return;
        }
...
// Definición de la lista de adyacencia en el formato deseado
$listaAdyacencia = [];

// A -> B (1), A -> C (4)
$listaAdyacencia[0][1] = 1; 
$listaAdyacencia[0][2] = 4; 

// B -> A (1), B -> C (2), B -> D (5), B -> E (7)
$listaAdyacencia[1][0] = 1; 
$listaAdyacencia[1][2] = 2; 
$listaAdyacencia[1][3] = 5; 
$listaAdyacencia[1][4] = 7; 

// C -> A (4), C -> B (2), C -> D (3), C -> F (1)
$listaAdyacencia[2][0] = 4; 
$listaAdyacencia[2][1] = 2; 
$listaAdyacencia[2][3] = 3; 
$listaAdyacencia[2][5] = 1; 

// D -> B (5), D -> C (3), D -> E (1), D -> G (6)
$listaAdyacencia[3][1] = 5; 
$listaAdyacencia[3][2] = 3; 
$listaAdyacencia[3][4] = 1; 
$listaAdyacencia[3][6] = 6; 

// E -> B (7), E -> D (1), E -> F (2), E -> H (3)
$listaAdyacencia[4][1] = 7; 
$listaAdyacencia[4][3] = 1; 
$listaAdyacencia[4][5] = 2; 
$listaAdyacencia[4][7] = 3; 

// F -> C (1), F -> E (2), F -> G (4), F -> I (5)
$listaAdyacencia[5][2] = 1; 
$listaAdyacencia[5][4] = 2; 
$listaAdyacencia[5][6] = 4; 
$listaAdyacencia[5][8] = 5; 

// G -> D (6), G -> F (4), G -> H (2), G -> J (3)
$listaAdyacencia[6][3] = 6; 
$listaAdyacencia[6][5] = 4; 

// H -> E (3), H -> G (2), H -> I (1)
$listaAdyacencia[7][8] = 1; 

// I -> F (5), I -> H (1), I -> J (4)
$listaAdyacencia[8][9] = 4; 
$listaAdyacencia[8][7] = 4; 

// J -> G (3), J -> I (4)
$listaAdyacencia[9][8] = 4; 

$nombresNodos = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J'];

$grafo = new Grafo($listaAdyacencia, $nombresNodos);
$nodosCaminos = $grafo->obtenerNodosEnCaminosMasCortos(7, 9, 3); // Cambia los índices según tus nodos


foreach ($nodosCaminos as $camino) {
	foreach ($camino as $nodo) {
		$miembros[$nodo]=$nombresNodos[$nodo];
	}
}
...
class Grafo {
    ...
    public function agregarVertice($nombre) {
        if (!isset($this->vertices[$nombre])) {
            $this->vertices[$nombre] = [];
        }
    }

    public function agregarArista($origen, $destino) {
        if (isset($this->vertices[$origen]) && !in_array($destino, $this->vertices[$origen])) {
            $this->vertices[$origen][] = $destino;
        }
    }

    public function imprimirGrafo() {
        foreach ($this->vertices as $vertice => $vecinos) {
            echo "Vertice: $vertice, Vecinos: " . implode(', ', $vecinos) . "\n";
        }
    }
}

function obtenerEnlaces($url, &$visitados, $grafo, $profundidad = 0) {
    // Limitar la profundidad de la búsqueda
    if ($profundidad > 2) {
        return;
    }

    // Verifica si ya se ha visitado esta URL
    if (isset($visitados[$url])) {
        return;
    }
    ...
    // Crea un nuevo DOMXPath para consultar el documento
    ...
    foreach ($enlaces as $enlace) {
        ...
        // Filtra enlaces internos de Wikipedia
        if (strpos($href, '/wiki/') === 0) {
            // Construye la URL completa
            $nuevaUrl = 'https://es.wikipedia.org' . $href;

            // Agrega el enlace al grafo
            $grafo->agregarVertice($url);
            $grafo->agregarVertice($nuevaUrl);
            $grafo->agregarArista($url, $nuevaUrl);

            // Llama recursivamente a la función para seguir explorando
            obtenerEnlaces($nuevaUrl, $visitados, $grafo, $profundidad + 1);
        }
    }
}

// URL inicial (puedes cambiarla a la que desees)
$urlInicial = 'https://es.wikipedia.org/wiki/Ludwig_van_Beethoven';
$visitados = [];
$grafo = new Grafo();

// Comienza el scraping
obtenerEnlaces($urlInicial, $visitados, $grafo);
