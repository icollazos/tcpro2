...
function buscaTextos($argumentos){
	...
	if($argumentos['frecuentes']!='0'){
		$clausulaFrecuentes = " AND aaa_texto.descriptor LIKE '%".$argumentos['frecuentes']."%' ";
	}
	...
	if($clausulaValor=="AND ( aaa_texto.descriptor LIKE '%%' ) "){
		unset($clausulaValor);
	};
	...
	INNER JOIN aaa_proyecto ON aaa_proyecto.idaaa_cliente=$cliente 
	$clausulaFecha
	$clausulaValor
	$clausulaFrecuentes
	AND aaa_texto.id>1 
