			...
			<div class="panel panel-default">
				<div class="panel-body" style="min-height: 100px;">
					<div class="row" style="margin-top: 24px; margin-bottom: 24px;">
						<div class="row">
							<label for="exampleFormControlTextarea1" class="form-label" style="margin-top: 10px;">Actor</label>
							<select class="form-select alto argumento-select" aria-label="Default select example" id="actor" onchange="getSelect();">
								<option value="7">SIETE</option>
								<? 
								foreach ($actores as $id=>$descriptor){
									echo '<option value="'.$id.'">'.$id.'. '.$descriptor.'</option>';
								}
								?>
							</select>
							<label for="exampleFormControlTextarea1" class="form-label" style="margin-top: 10px;">Actor 2</label>
							<select class="form-select alto argumento-select" aria-label="Default select example" id="actor2" onchange="getSelect();">
							...
							<select class="form-select alto argumento-select" multiple aria-label="Default select example" id="relaciones" onchange="getSelect();" style="height: 200px;">
								<? 
...
<script>
	...
	async function postData(url = 'caminos_select.php', data = {}) {
		l("Enviando datos");
	...
	function getSelect(){
		...
		if (argumentos['actor'] && argumentos['actor2'] && argumentos['relaciones']) {
			postData('caminos_select.php', { 
				actor:argumentos['actor'], 
				actor2:argumentos['actor2'], 
			...
			.then(data => {
				...
				//const filteredData = data.filter(d => d.nodes.activo == 1);
				grafo=data.grafo;

				const filteredNodes = grafo.nodes.filter(node => node.activo === 1);
