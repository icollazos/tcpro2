...
function buscaData($argumentos){
	...
	if($clausulaKw=="AND ( aaa_texto.descriptor LIKE '%%')"){
		$clausulaKw='';
	}

	$excepto=$argumentos['excepto'];
	$excepto=explode(",",$excepto);
	foreach ($excepto as $key => $value) {
		$ex[]=trim($value);
	}
	if(count($ex)!=0){
		$clausulaEx="AND ( aaa_texto.descriptor NOT LIKE '%";
		$clausulaEx.=implode("%' AND aaa_texto.descriptor NOT LIKE '%", $ex);
		$clausulaEx.="%')";
	} 
	if($clausulaEx=="AND ( aaa_texto.descriptor NOT LIKE '%%')"){
		$clausulaEx='';
	}

	$sql="SELECT SUM(aaa_lemaPar.relevancia) as peso, aaa_lemaPar.lema1 as lema1,  aaa_lemaPar.lema2 as lema2, aaa_lemaPar.descriptor as par 
	FROM aaa_lemaPar
	INNER JOIN aaa_texto ON aaa_texto.id=aaa_lemaPar.idaaa_texto 
	$clausulaFecha
	$clausulaKw 
	$clausulaEx 
	AND aaa_lemaPar.lop='P'
	AND aaa_texto.id IN (
		SELECT 
		aaa_textoValor.idaaa_texto 
		FROM aaa_textoValor 
		...
		if ($result->num_rows> 0){
			while ($row = $result->fetch_assoc()){
				if($row['peso']>5){
					...
					$pesos[$row['par']]=$row['peso'];
					$pesos[$row['lema1']]+=$row['peso'];
					$pesos[$row['lema2']]+=$row['peso'];
					$i++;					
				}
			}
		}
...
$ref=10000;
$c=explode(',','red,yellow,lightgrey');
foreach ($pesos as $key => $value) {
	...
	$pesos[$key]['peso']=$p;
	$colores[$key]=$c[2];
	if($p>$max*0.3){$colores[$key]=$c[1];}
	if($p>$max*0.6){$colores[$key]=$c[0];}
}
...
sort($lista);
$listaEspejo=array_flip($lista); 

$i=0;
foreach ($lista as $key => $value) {
	$nodes[$i]['id']=$i;
	$nodes[$i]['nodeName']=$value;
	$nodes[$i]['peso']=log(pow($pesos[$value],2));
	$nodes[$i]['color']=$colores[$value];
	$nodes[$i]['group']=10;
	$i++;
}
$i=0;
foreach ($t as $key => $value) {
		$links[$i]['source']=$listaEspejo[$value['lema1']];
		$links[$i]['target']=$listaEspejo[$value['lema2']];
		$links[$i]['value']=log(pow($pesos[$value['par']],1)); 
		if($links[$i]['value']<1){$links[$i]['value']=1;}
		$i++;
}
